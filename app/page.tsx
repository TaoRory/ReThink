  ...
  return (
    <>
      ...
      <section className="smoke grain guides relative overflow-hidden pt-16">
        ...
        <Starfield />
        <PointerGlow />
        <Sparkle className="absolute left-[8%] top-28 h-10 w-10 text-lavender-300/70" />
        <Sparkle className="absolute right-[10%] top-48 h-16 w-16 text-lavender-400/50" />
        <Sparkle className="absolute bottom-24 left-[16%] h-6 w-6 text-pink-200/60" />

        <div className="mx-auto flex min-h-[92vh] max-w-6xl flex-col items-center justify-center px-5 py-24 text-center">
          <p className="rise mb-6 text-xs font-semibold uppercase tracking-[0.5em] text-lavender-300/80">
            We are…
          </p>

          <h1 className="rise flex items-baseline justify-center leading-none [animation-delay:0.1s]">
            <span className="font-script text-chrome text-[clamp(6rem,17vw,13rem)] -mr-2">
              Re
            </span>
            <span className="font-display text-chrome text-[clamp(3.4rem,10vw,7.5rem)] font-bold tracking-[0.12em]">
              THINK
            </span>
          </h1>

          <p className="rise mt-4 text-sm font-semibold uppercase tracking-[0.45em] text-lavender-300 [animation-delay:0.2s]">
            Since 2022 · VinUniversity
          </p>

          <p className="rise mx-auto mt-10 max-w-2xl font-display text-xl leading-relaxed text-cream-100/90 italic sm:text-2xl [animation-delay:0.3s]">
            “If you want a place to gather{" "}
            <span className="text-lavender-300">diverse ideas</span> and take
            them to drive{" "}
            <span className="text-lavender-300">social innovation</span>, you
            come to ReThink.”
          </p>
        ...
        {/* value marquee */}
        <div className="relative border-y border-white/10 bg-plum-900/60 py-4 overflow-hidden">
          <div className="marquee-track flex w-max gap-12 whitespace-nowrap">
            {[...Array(2)].map((_, i) => (
              <div key={i} className="flex gap-12 text-sm font-medium uppercase tracking-[0.3em] text-cream-100/60">
      ...
      <section className="relative overflow-hidden py-24">
        <div className="mx-auto grid max-w-6xl items-center gap-14 px-5 lg:grid-cols-2">
          ...
          <div className="relative" data-reveal style={{ "--reveal-delay": "0.15s" } as React.CSSProperties}>
            ...
            <Sparkle className="absolute -right-5 -top-6 h-12 w-12 text-lavender-300/80" />
            <Sparkle className="absolute -bottom-5 -left-4 h-8 w-8 text-pink-200/70" />
          </div>
        </div>
      </section>
      ...
      <section className="smoke grain relative overflow-hidden border-y border-white/8 py-24">
        <div className="mx-auto max-w-6xl px-5">
          <SectionHeading
            ...
            className="text-center [&>h2]:justify-center"
          />
          <div className="mt-14 grid gap-5 sm:grid-cols-2" data-reveal>
            {coreValues.map((v, i) => (
              <div
                key={v.title}
                className="frame-dashed group relative bg-plum-900/50 p-7 backdrop-blur-sm transition hover:bg-plum-800/60"
              >
                <span className="font-script text-4xl text-lavender-300/90">
                  {String(i + 1).padStart(2, "0")}
                </span>
                <h3 className="mt-2 font-display text-xl font-semibold text-cream-100">
                  {v.title}
                </h3>
                ...
                <Sparkle
                  animate={false}
                  className="absolute right-5 top-5 h-4 w-4 text-lavender-400/40 transition group-hover:text-lavender-300"
                />
              </div>
            ))}
          </div>
          ...
          <div className="mt-14 grid gap-6 md:grid-cols-3" data-reveal>
            {activities.map((a) => {
              const card = (
                <article
                  key={a.title}
                  className="group relative overflow-hidden rounded-2xl border border-white/10 bg-plum-900/60 transition hover:border-lavender-400/40"
                >
                  ...
                  <div className="p-6">
                    ...
                    <h3 className="mt-2 font-display text-2xl font-semibold text-cream-100">
                      {a.title}
                    </h3>
                    <p className="mt-3 text-sm leading-relaxed text-cream-100/70">
                      {a.desc}
                    ...
                    {a.href && (
                      <p className="mt-4 text-sm font-semibold text-lavender-300 group-hover:text-lavender-200">
                        Khám phá TEDx →
                      </p>
                    )}
                  </div>
      ...
      <section className="relative overflow-hidden border-y border-white/8">
        <div className="absolute inset-0">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-r from-plum-950 via-plum-950/85 to-crimson-700/40" />
        </div>
        <div className="relative mx-auto max-w-6xl px-5 py-24" data-reveal>
          ...
          <p className="mt-5 max-w-xl text-[15px] leading-relaxed text-cream-100/80">
            Với chủ đề <strong className="text-cream-100">“The Firsts”</strong>,
          ...
          <div className="mt-10 grid max-w-2xl grid-cols-2 gap-6 sm:grid-cols-4">
            {[
              ...
              <div key={l}>
                <p className="font-display text-4xl font-bold text-cream-100">{n}</p>
                <p className="mt-1 text-xs uppercase tracking-[0.2em] text-cream-100/60">{l}</p>
              </div>
            ))}
          </div>
        ...
        <div className="mx-auto max-w-6xl px-5">
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-3" data-reveal>
            {future.map((f) => (
              <div
                key={f.num}
                className="relative rounded-2xl border border-white/10 bg-plum-900/50 p-7 backdrop-blur-sm"
              >
                <span className="font-display text-5xl font-bold text-violet-500/50">
                  {f.num}
                </span>
                <h3 className="mt-3 font-display text-xl font-semibold text-lavender-200">
                  {f.title}
                </h3>
                <p className="mt-3 text-sm leading-relaxed text-cream-100/70">
                  {f.desc}
      ...
      <section className="relative overflow-hidden border-t border-white/8 py-28 text-center">
        ...
        <Sparkle className="absolute left-[12%] top-14 h-8 w-8 text-lavender-300/60" />
        <Sparkle className="absolute right-[14%] bottom-16 h-12 w-12 text-lavender-400/50" />
        <div className="relative mx-auto max-w-3xl px-5" data-reveal>
          <h2 className="font-display text-4xl leading-tight text-cream-100 sm:text-5xl">
            <span className="font-script text-chrome text-[1.6em] align-[-0.12em] mr-2">L</span>
            et&apos;s Rethink.
            <br />
            Let&apos;s be{" "}
